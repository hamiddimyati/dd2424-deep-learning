...
class NeuralNetwork():
    def __init__(self, file='datasets/cifar-10-batches-py/batches.meta'):
        """
        Parameters:
    ...
    def fit(self, X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_, n_batch, eta, n_epoch, type_norm):
        """
        Parameters:
        ...
        self.mini_batch_GD(X_train_norm, Y_train, y_train, X_val_norm, Y_val, y_val, lambda_, n_batch, eta, n_epoch)
    ...
    def mini_batch_GD(self, X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_, n_batch, eta, n_epoch):
        """
        Parameters:
            X_train (d, n_1): image pixel training data
            Y_train (K, n_1): one-hot representation of the label for each training image
...
if __name__ == "__main__":
    ...
    #test method
    model_NN.test_method(X_train, Y_train, lambda_=0, n_batch=100, ht=1e-4, type_norm=norm)
    ...
    list_lambda = [0, 0, 0.1, 1]
    n_batch = 100
    n_epoch = 40
    list_eta = [0.1, 0.001, 0.001, 0.001]
    for i in range(len(list_lambda)):
        model_NN.fit(X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_=list_lambda[i], n_batch=n_batch, eta=list_eta[i], n_epoch=n_epoch, type_norm=norm)
        ...
        fname = "lambda:{}, n_batch:{}, n_epoch:{}, eta:{}".format(list_lambda[i], n_batch, n_epoch, list_eta[i])
        model_NN.plotting(n_epoch=range(n_epoch), filename=fname)
