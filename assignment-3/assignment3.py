...
class NeuralNetwork():
    def __init__(self, n_hidden, batch_normalization, init_method, file='datasets/cifar-10-batches-py/batches.meta'):
        """
        Parameters:
    ...
    def fit(self, X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_, n_batch, n_epoch, n_step, eta_min, eta_max, sig, shuffle, cost, cyclical, type_norm):
        """
        Parameters:
    ...
    def predict(self, X, y):
        ...
        """
        final_acc = self.compute_accuracy(X, y, train_mode=False)
        return final_acc
    ...
    def evaluate_classifier(self, X, train_mode):
        """
        Parameters:
            X (d, n): image pixel data
    ...
    def compute_gradients(self, X, Y, lambda_):
        ...
        if self.batch_normalization:
            ...
            all_output = self.evaluate_classifier(X, train_mode=True)
    ...
    def mini_batch_GD(self, X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_, n_batch, n_epoch, n_step, eta_min, eta_max, shuffle, cost, cyclical):
        ...
        for i in range(n_epoch):
            ...
            for id in range(n // n_batch):
                ...
                grads = self.compute_gradients(X_batch, Y_batch, lambda_)
                ...
                if cost & (t % rec == 0):
                    ...
                    acc_train = self.compute_accuracy(X_train, y_train, train_mode=True)
                    acc_val = self.compute_accuracy(X_val, y_val, train_mode=True)
    ...
    def compute_accuracy(self, X, y, train_mode):
        """
        Parameters:
            X (d, n): image pixel data
            y (n): label for each image
        ...
        """
        n = X.shape[1]
        all_output = self.evaluate_classifier(X, train_mode)
        P = all_output['P']
...
if __name__ == "__main__":
    ...
    #test method
    hid_nodes = [50, 50, 50]
    model_NN = NeuralNetwork(n_hidden=hid_nodes, batch_normalization=False, init_method='He')
    model_NN.test_method(X_train, Y_train, lambda_=0, n_batch=20, type_norm=norm)
    model_NN.fit(X_train[:,:100], Y_train[:,:100], y_train[:100], X_val[:,:100], Y_val[:,:100], y_val[:100], lambda_=0, n_batch=100, n_epoch=200, n_step=5, eta_min=1e-1, eta_max=1e-1, sig=1e-1, shuffle=True, cost=True, cyclical=False, type_norm=norm)
    plt.plot(range(200), model_NN.cost_train)
    plt.xlabel('Epoch')
    plt.ylabel('Training Cost')
    plt.title('Training Cost by Epoch')
    plt.savefig('epoch vs cost {} no-BN.png'.format(str(hid_nodes)))
    plt.close()
    ...
    BN=False
    for i in range(2):
        model_NN = NeuralNetwork(n_hidden=[50], batch_normalization=BN, init_method='He')
        model_NN.fit(X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_=lambda_, n_batch=n_batch, n_epoch=n_epoch[i], n_step=n_step[i], eta_min=eta_min, eta_max=eta_max, sig=1e-1, shuffle=True, cost=True, cyclical=True, type_norm=norm)
    ...
    model_NN.fit(X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_=lambda_, n_batch=n_batch, n_epoch=n_epoch, n_step=n_step, eta_min=eta_min, eta_max=eta_max, sig=1e-1, shuffle=True, cost=True, cyclical=True, type_norm=norm)
    ...
    n_step=5 * 45000 / n_batch
    lambda_=0.005
    hid_nodes=[50, 30, 20, 20, 10, 10, 10, 10]
    BN = True
    model_NN = NeuralNetwork(n_hidden=hid_nodes, batch_normalization=BN, init_method='He')
    model_NN.fit(X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_=lambda_, n_batch=n_batch, n_epoch=n_epoch, n_step=n_step, eta_min=eta_min, eta_max=eta_max, sig=1e-1, shuffle=True, cost=True, cyclical=True, type_norm=norm)
    print('training accuracy: {}'.format(model_NN.predict(X_train_norm, y_train)))
    ...
    lambda_ = 0.00440015
    n_epoch=8
    n_step=2 * 45000 / n_batch
    hid_nodes=[50, 50]
    BN = [False, True]
    sig = [1e-1, 1e-3, 1e-4]
    for j in range(len(BN)):
        for i in range(len(sig)):
            model_NN = NeuralNetwork(n_hidden=hid_nodes, batch_normalization=BN[j], init_method=None)
            model_NN.fit(X_train, Y_train, y_train, X_val, Y_val, y_val, lambda_=lambda_, n_batch=n_batch, n_epoch=n_epoch, n_step=n_step, eta_min=eta_min, eta_max=eta_max, sig=sig[i], shuffle=True, cost=True, cyclical=True, type_norm=norm)
            print('training accuracy BN {}, sig {}: {}'.format(str(BN[j]), sig[i], model_NN.predict(X_train_norm, y_train)))
            print('validation accuracy BN {}, sig {}: {}'.format(str(BN[j]), sig[i], model_NN.predict(X_val_norm, y_val)))
            print('testing accuracy BN {}, sig {}: {}'.format(str(BN[j]), sig[i], model_NN.predict(X_test_norm, y_test)))
            fname = "BN:{}, sig:{}, lambda:{}, n_batch:{}, n_epoch:{}, eta:{}, n_step:{}".format(str(j), i, lambda_, n_batch, n_epoch, 'cyclic', n_step)
            model_NN.plotting(filename=fname)
