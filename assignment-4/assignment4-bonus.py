...
def tweet_cleaning(data):
  for i in range(len(data)):
    data[i]['text'] = re.sub(r"http\S+", "", data[i]['text']).strip()
    data[i]['text'] = re.sub("[^a-zA-Z0-9\s\.\,\!\?\@]", "", data[i]['text'])
  return data
...
def load_data(file_name):
    ...
    """
    with open('condensed_2018.json') as file:
        data_json = json.load(file)
    data_json = tweet_cleaning(data_json)
    data = "".join(d['text'] for d in data_json)
    chars = list(set(data))
    K = len(chars)
    ...
    def AdaGrad(self, compile_output, seq_len, epoch, eta):
        ...
        for i in range(epoch):
            ...
            for j in range(nb_seq):
                ...
                if t % 10000 == 0:
                    Y_t = self.synthesize_text(hprev, X[:, 0], 140)
                    text = ''
...
if __name__ == '__main__':
    start = time.time()
    data = load_data('condensed_2018.json')
    sig = 0.01
    seq_len = 50
    epoch = 7
    eta = 0.1
    ...
    plt.savefig('epoch vs loss for {} epochs, {} sequence length.png'.format(epoch, seq_len))
    plt.close()